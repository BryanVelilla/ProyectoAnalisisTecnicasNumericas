...
export interface GeometricFitRow {
  n: number;
  x: number;
  y: number;
  lnX: number;
...
export interface GeometricFitResult {
  a: number;
  b: number;
  r: number;
  rSquared: number;
  ...
  sumLnY2: number;
  sumLnXlnY: number;
  rows: GeometricFitRow[];
  invalidPoints: boolean; // xi <= 0 or yi <= 0
}
...
export const GeometricFitService = {
  /**
   * Fits y = a·x^b by linearising: ln(y) = ln(a) + b·ln(x)
   * Requires xi > 0 and yi > 0 for all points.
   */
  compute(points: DataPoint[]): GeometricFitResult | null {
    ...
    const rows: GeometricFitRow[] = points.map((p, i) => {
  ...
  getFitCurve(result: GeometricFitResult, points: DataPoint[], count = 120): { x: number; y: number }[] {
