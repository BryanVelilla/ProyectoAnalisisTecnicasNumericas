...
import { Cpu, Download, RefreshCw, Info, AlertTriangle, Zap } from 'lucide-react';
import {
  ScatterChart, Scatter, XAxis, YAxis, CartesianGrid,
  Tooltip, Legend, ResponsiveContainer, Label,
} from 'recharts';
import { useData } from '../../../store/DataContext';
import { PowerFitService, type PowerFitResult } from '../../../services/powerFit.service';
import { DataService } from '../../../services/data.service';
import { getMethodById } from '../../../constants/methods';
import { PageHeader } from '../../../components/shared/PageHeader';
import { Button } from '../../../components/ui/Button';
import { Badge } from '../../../components/ui/Badge';
import styles from './PowerFit.module.css';
...
const method = getMethodById('power')!;
...
type ExponentClass = 'superlinear' | 'linear' | 'sublinear' | 'inverse';

function classifyExponent(b: number): { cls: ExponentClass; label: string; desc: string } {
  if (b > 1)          return { cls: 'superlinear', label: 'Superlineal (b > 1)',    desc: 'Crecimiento más rápido que lineal. La curva es convexa y se acelera.' };
  if (Math.abs(b - 1) < 1e-6) return { cls: 'linear', label: 'Lineal (b = 1)',    desc: 'Relación directamente proporcional: y = a·x.' };
  if (b > 0)          return { cls: 'sublinear',  label: 'Sublineal (0 < b < 1)',  desc: 'Crecimiento más lento que lineal. La curva es cóncava y se desacelera.' };
  return               { cls: 'inverse',      label: 'Potencia inversa (b < 0)', desc: 'Relación inversamente proporcional: y decrece al aumentar x.' };
}
...
function PowerChart({ result, dataPoints, fitCurve }: ChartProps) {
    ...
    <ResponsiveContainer width="100%" height={340}>
      <ScatterChart margin={{ top: 16, right: 24, bottom: 32, left: 16 }}>
        ...
        <Scatter
          ...
          data={fitCurve}
          fill="transparent"
          line={{ stroke: '#ea580c', strokeWidth: 2 }}
...
function CalcTable({ result }: { result: PowerFitResult }) {
  return (
...
function LinearizationBox({ result }: { result: PowerFitResult }) {
...
// ─── Power-law analysis card ──────────────────────────────────────────────────

function PowerLawAnalysis({ result }: { result: PowerFitResult }) {
  const { cls, label, desc } = classifyExponent(result.b);
  const doublingFactor = Math.pow(2, result.b);
  const tenFactor      = Math.pow(10, result.b);
  const yAtOne         = result.a;

  return (
    <div className={styles.card}>
      <p className={styles.cardTitle} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <Zap size={15} />Análisis de Ley de Potencia
      </p>
      <div className={`${styles.exponentBadge} ${styles[cls]}`}>
        {label}
      </div>
      <div className={styles.powerGrid}>
        <div className={styles.powerItem}>
          <span className={styles.powerLabel}>Comportamiento del exponente</span>
          <span className={styles.powerValue}>b = {fmtResult(result.b)}</span>
          <span className={styles.powerDesc}>{desc}</span>
        </div>
        <div className={styles.powerItem}>
          <span className={styles.powerLabel}>y cuando x = 1</span>
          <span className={styles.powerValue}>{fmtResult(yAtOne)}</span>
          <span className={styles.powerDesc}>Siempre igual al coeficiente a, ya que 1^b = 1.</span>
        </div>
        <div className={styles.powerItem}>
          <span className={styles.powerLabel}>Escala al doblar x</span>
          <span className={styles.powerValue}>× {fmtResult(doublingFactor)}</span>
          <span className={styles.powerDesc}>
            Si x → 2x, entonces y se multiplica por 2<sup>b</sup> = {fmtResult(doublingFactor)}.
          </span>
        </div>
        <div className={styles.powerItem}>
          <span className={styles.powerLabel}>Escala al multiplicar x × 10</span>
          <span className={styles.powerValue}>× {fmtResult(tenFactor)}</span>
          <span className={styles.powerDesc}>
            Si x → 10x, entonces y se multiplica por 10<sup>b</sup> = {fmtResult(tenFactor)}.
          </span>
        </div>
      </div>
    </div>
...
function buildPowerSteps(result: PowerFitResult): StepData[] {
  ...
  return [
    {
      ...
      children: (
        <div>
          <p className={ss.text}>
            El modelo y = ax^b se linealiza aplicando logaritmo natural a ambos lados:
          </p>
          <div className={ss.formulaBox} style={{ borderLeftColor: C }}>
            ln(y) = ln(a) + b·ln(x) &nbsp;→&nbsp; Y = A + b·X &nbsp;(X = ln(x), Y = ln(y), A = ln(a))
          </div>
          <p className={ss.note}>
            A diferencia del ajuste geométrico, aquí b se interpreta como exponente de la ley de potencia
            (b&gt;1 superlineal, b≈1 lineal, 0&lt;b&lt;1 sublineal, b&lt;0 inverso).
          </p>
        </div>
      ),
    },
    ...
    {
      title: 'Exponente b (pendiente en log-log)',
      children: (
        <div>
...
export function PowerFit() {
  ...
  const result = useMemo(
    () => (calculated && points.length >= 2 ? PowerFitService.compute(points) : null),
    [calculated, points]
  );

  const fitCurve = useMemo(
    () => (result && !result.invalidPoints ? PowerFitService.getFitCurve(result, points) : []),
    [result, points]
  );
  ...
  function handleExport() {
    if (!result) return;
    const lines = [
      'AJUSTE POTENCIAL — CurveAnalysis',
      '==================================',
      `n = ${result.n}`,
    ...
    a.href = url;
    a.download = 'ajuste_potencial.txt';
    a.click();
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button variant="primary" size="sm" leftIcon={<Cpu size={14} />}
      ...
      {hasData && (
        <div className={styles.layout}>
          ...
          <div className={styles.mainCol}>
            ...
            {result?.invalidPoints && (
              <div className={styles.warningBanner}>
                ...
                <span>
                  El ajuste potencial requiere Xi &gt; 0 y Yi &gt; 0 en todos los pares.
            ...
            <div className={styles.card}>
              <div className={styles.cardHeader}>
                <div>
                  <p className={styles.cardTitle}>Gráfica — Ajuste Potencial</p>
                  <p className={styles.cardSub}>Modelo: {method.formula}</p>
                </div>
              </div>
              {result && !result.invalidPoints ? (
                <PowerChart result={result} dataPoints={dataPoints} fitCurve={fitCurve} />
              ) : (
                <div className={styles.chartPlaceholder}>
                  <Cpu size={28} className={styles.placeholderIcon} />
                  <p>
                    {result?.invalidPoints
            ...
            {result && !result.invalidPoints && (
              <SolutionSteps steps={buildPowerSteps(result)} color="#ea580c" />
            )}
          ...
          <div className={styles.sideCol}>
            ...
            {result && !result.invalidPoints && (
              <>
                <PowerLawAnalysis result={result} />
            ...
            <div className={styles.card}>
              ...
              <p className={styles.infoNote}>
                ...
                ln(y) = ln(a) + b·ln(x). A diferencia del modelo exponencial, el exponente b
                actúa sobre la base x, no sobre la función.
              </p>
            </div>
